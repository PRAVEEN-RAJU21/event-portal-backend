...
public class EventController {
    ...
    public ResponseEntity<?> createBooking(@RequestBody Booking bookingRequest) {
        ...
        if (reqEventId == null) {
            return ResponseEntity.badRequest().body("Error: Event ID is missing.");
        }
        ...
        if (eventOptional.isPresent()) {
            ...
            if (event.isGroupEvent() && bookingRequest.getTicketsBooked() > event.getMaxGroupSize()) {
                return ResponseEntity.badRequest().body("Error: Group size exceeds limit.");
            }
            ...
            try {
                ...
                byte[] pdfTicket = fileService.generateTicketPDF(
                    savedBooking.getUserName(), event.getEventName(), event.getVenue(), formattedDate, qrCodeBase64
                );

                emailService.sendTicketEmail(
                    savedBooking.getEmailId(), savedBooking.getUserName(), event.getEventName(),
                    savedBooking.getTicketsBooked(), event.getVenue(), formattedDate,
                    savedBooking.getTotalAmount().toString(), qrCodeBase64, pdfTicket, event.getCategory() 
                );
            } catch (Exception e) {
                System.err.println("Booking success, but assets failed: " + e.getMessage());
            }
